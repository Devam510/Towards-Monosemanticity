...
If $\mathcal{L}_{\mathrm{consist}}$ synchronises all $L$ layers into a single global rotation frame, anchoring layer 0 to the static embedding matrix $W_E$ selects the unique true global rotation.
\end{lemma}
\begin{proof}
Adding $\mathcal{L}_{\mathrm{anchor}} = \|W_{\Dec}^{(0)} - W_E^\top\|_F^2$ locks the rotational freedom of layer 0 to fixed semantic concepts encoded in $W_E$. By Lemma 2, all subsequent layers are then uniquely determined.
\end{proof}

\begin{proposition}[\CCJFR{} Identifiability]
Given $\gamma > 0$ and boundary anchoring, \CCJFR{} yields a uniquely identifiable multi-layer feature recovery up to permutation of equivalent features.
\end{proposition}
\begin{proof}
